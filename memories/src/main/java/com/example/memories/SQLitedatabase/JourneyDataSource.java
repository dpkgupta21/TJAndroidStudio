    ...
    private static List<Journey> parseJourneysAsList(Context context, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            while (!cursor.isAfterLast()) {
                ...
                journey.setCreatedAt(cursor.getLong(cursor.getColumnIndex(MySQLiteHelper.JOURNEY_COLUMN_CREATED_AT)));
                journey.setUpdatedAt(cursor.getLong(cursor.getColumnIndex(MySQLiteHelper.JOURNEY_COLUMN_UPDATED_AT)));
                journey.setCompletedAt(cursor.getLong(cursor.getColumnIndex(MySQLiteHelper.JOURNEY_COLUMN_COMPELTED_AT)));
